...
DocGenerator::DocGenerator()
    ...
    LOG_INFO("DocGenerator initialized");
}
    ...
    for (int i = 0; i < lines.size(); i++) {
        ...
        if (inDocBlock) {
            ...
            if (paramMatch.hasMatch()) {
                ...
                QString paramType = paramMatch.captured(3);
            ...
            if (returnMatch.hasMatch()) {
                currentItem.returnType = returnMatch.captured(2);
                if (!returnMatch.captured(4).isEmpty()) {
                    currentItem.description += "\nReturns: " + returnMatch.captured(4);
                }
            }
            ...
            if (optionMatch.hasMatch()) {
                QString optName = optionMatch.captured(2);
                QString optType = optionMatch.captured(3);
                QString optDesc = optionMatch.captured(4);
