...
/**
 * @brief Вариант значения для валидации (аналог QVariant)
 */
using ValidationValue = std::variant<
    ...
    std::string,        // строка
    std::vector<ValidationValue>,  // вектор/список
    std::map<std::string, ValidationValue>  // ассоциативный массив
>;
...
struct ValidationRule {
    ...
    ValidationValue parameters;     // Параметры валидатора
    std::string errorMessage;       // Сообщение об ошибке
    bool isOptional;                // Необязательный аргумент
    ValidationValue defaultValue;   // Значение по умолчанию
...
struct ValidationResult {
    ...
    std::string argumentName;   // Имя аргумента
    std::string errorMessage;   // Сообщение об ошибке
    ValidationValue validatedValue;    // Валидированное значение
    ValidatorType validator;    // Использованный валидатор
    
    ValidationResult() : isValid(true), validator(ValidatorType::Custom) {}
    
    static ValidationResult success(const std::string& argName, const ValidationValue& value) {
...
public:
    ...
    std::vector<ValidationResult> validateArguments(
        ...
        const std::vector<ValidationValue>& actualValues,
     ...
     */
    ValidationResult validateArgument(const ValidationRule& rule, const ValidationValue& value);
     ...
     */
    static bool isLogical(const ValidationValue& value);
     ...
     */
    static bool isPositive(const ValidationValue& value);
     ...
     */
    static bool isNegative(const ValidationValue& value);
     ...
     */
    static bool isNonNegative(const ValidationValue& value);
     ...
     */
    static bool isInteger(const ValidationValue& value);
     ...
     */
    static bool isReal(const ValidationValue& value);
     ...
     */
    static bool isNumeric(const ValidationValue& value);
     ...
     */
    static bool isString(const ValidationValue& value);
     ...
     */
    static bool isVector(const ValidationValue& value);
     ...
     */
    static bool isMatrix(const ValidationValue& value);
     ...
     */
    static bool isLayer(const ValidationValue& value);
     ...
     */
    static bool isCollection(const ValidationValue& value);
     ...
     */
    static bool isEmpty(const ValidationValue& value);
     ...
     */
    static bool isNotEmpty(const ValidationValue& value);
     ...
     */
    static bool hasLength(const ValidationValue& value);
     ...
     */
    static bool hasSize(const ValidationValue& value);
     ...
     */
    static bool isInRange(const ValidationValue& value, double min, double max);
     ...
     */
    static bool isOneOf(const ValidationValue& value, const std::vector<ValidationValue>& allowedValues);
     ...
     */
    static bool matchesPattern(const ValidationValue& value, const std::string& pattern);
     ...
     */
    static bool isClass(const ValidationValue& value, const std::string& className);
     ...
     */
    static bool isInterface(const ValidationValue& value, const std::string& interfaceName);
     ...
     */
    static bool isCallable(const ValidationValue& value);
     ...
     */
    static bool isIterable(const ValidationValue& value);
    ...
    void registerCustomValidator(const std::string& name, 
                                 std::function<bool(const ValidationValue&)> validatorFunc);
     ...
     */
    bool executeCustomValidator(const std::string& name, const ValidationValue& value);
     ...
     */
    std::string generateErrorMessage(const ValidationRule& rule, const ValidationValue& value);
...
private:
     ...
     * @return Параметры
     */
    ValidationValue extractAnnotationParameters(const std::string& annotationText);
    ...
    std::map<std::string, std::function<bool(const ValidationValue&)>> customValidators;
